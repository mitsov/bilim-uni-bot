var GeographyGame = (function() {
    ...
    function init(){
        ...
        for (i=1; i<Object.keys($Geography).length; i++){
            ...
            if ($Geography[i].value.continent != "Australia and Oceania"){
    ...
    function getQuestionType5(){
        ...
        if (value.continent_part) {
            ...
            if ($session.continent_part === 'Europe') {
    ...
    function getAnswerCountry($parseTree){
        ...
        if ($parseTree.Country && $parseTree.Country[1] && ($parseTree.Country[0].value.name != $parseTree.Country[1].value.name)) {
            ...
            if ($parseTree.Country[0].text.toLowerCase() == "america") {
                if ($session.rightAnswer == "Africa" || $session.rightAnswer == "Eurasia") {
                    $session.nextState = "WrongAnswer";
                } else {
                    ...
                    if ($parseTree.Country[0].text.toLowerCase() == "australia") {
                        if ($session.rightAnswer == "Australia") 
    ...
    function getAnswerContinent($parseTree){
        ...
        if ($parseTree.Continent && $parseTree.Continent[1]) {
            ...
            if ($session.rightAnswer == "Africa"){
                condition = ($parseTree.Continent[0].value == $session.rightAnswer);
            } else if ($session.rightAnswer == "Eurasia"){
                if ($parseTree.Continent[0].value === 'Europe' || $parseTree.Continent[0].value === 'Asia') {
                ...
                if ($parseTree.Continent && $parseTree.Continent[0].value == "Америка") {
                    ...
                    var answ;
                    if($parseTree.AmericaDescr[0].value == "North_America"){
                        answ = "North America";
                    }
                    else{
                        answ = "South America";
                    ...
                    var answ;
                    if($parseTree.Continent[0].value == "North_America"){
                        answ = "North America";
                    }
                    else{
                        answ = "South America";
                    ...
                    condition = ("North America" == $session.rightAnswer);  
                }
            } 
        }
        $session.nextState = (condition) ? "RightAnswer" : "WrongAnswer";    
